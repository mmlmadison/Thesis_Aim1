...
PSdata <- read.csv("Preschool_Volume_Longitudinal_June2021.csv")
...
######### Results Log File
sink("PSVolumeLog_Raw_NoCoV_June2021", append=FALSE, split=TRUE)

######### All MaCRUISE regions, not normalized 
regions <-as.data.frame(PSdata[,c(14:145)])
dim(regions)
...
for (i in 1:length(regions)) { 
  ...
  if (finalmodel == lmer_volume_null) {
    print("NULL")
  }
...
sink("PSVolumeLog_Norm_NoCoV_June2021", append=FALSE, split=TRUE)
#magic_for(silent = TRUE)
########## All MaCRUISE Regions, normalized by ICV
regions <-as.data.frame(PSdata[,c(146:277)])
dim(regions)
...
sink("PSVolumeLog_Raw_Sex_June2021", append=FALSE, split=TRUE)
...
sink("PSVolumeLog_Norm_Sex_QuadCub_July2021", append=FALSE, split=TRUE)
...
sink("PSVolumeLog_Raw_MatEdu_July2021", append=FALSE, split=TRUE)
...
sink("PSVolumeLog_Norm_SES_June2021", append=FALSE, split=TRUE)
